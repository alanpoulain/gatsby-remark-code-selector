...
remarkMacro.addMacro(
  "codeSelector",
  (content, props, { transformer, eat }) => ({
...
module.exports = ({ markdownAST }) => {
  visit(markdownAST, "CodeSelectorNode", (node) => {
    ...
    node.children.forEach((codeNode) => {
      ...
      if (codeNode.type !== "html" || !codeNode.lang) {
        console.warn(
          "content inside [codeSelector] should only be parsed code, received:\n",
          codeNode
        );
        return;
      }
      html += `<div class="code-selector-code-${codeNode.lang}" style="${
        firstCodeNode ? "" : "display: none"
      }">${codeNode.value}</div>`;
      toolbarHtml += `<button type="button" class="code-selector-select-${
        codeNode.lang
      } ${firstCodeNode ? "selected" : ""}">${codeNode.lang}</button>`;
      firstCodeNode = false;
    });
